...
/*中间搜索框的滚动距离大于距离顶部的距离，搜索框固定*/
$(function(){
	var _defaultTop = $('.search-box').offset().top;
	$(window).scroll(function(){
		if($(this).scrollTop()> _defaultTop){
			$('.search-box').addClass('fixed').css('backgroundColor','#C7C7C7');
		}else{
			$('.search-box').removeClass('fixed').css('backgroundColor','#EFEFEF');
		}
	});
});
/*字符串获取值*/
